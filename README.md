...
### Install the PulseTile User Interface

Copy the PulseTile UI files to the NGINX Web Server root directory:

       ~/usr/share/nginx/html

You can either create a build copy of the UI files from their source at:

      https://github.com/PulseTile/PulseTile-React-Core

Alternatively, the Ripple-QEWD-MicroServices repo contains a pre-built copy of the files (though they may not be the most up to date versions) in the folder :

      ~/helm/DMZ/usr/share/nginx/html
